...
import { LauncherSettings, Account } from '../../types';
...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({
  ...
  const [selectedTheme, setSelectedTheme] = useState<LauncherSettings['theme']>(settings.theme || 'nebula-purple');
  ...
  const themes: {
  ...
  }[] = [
    {
      id: 'nebula-purple',
      name: 'Nebula Purple',
      desc: 'Deep cosmic violet & ethereal indigo',
    ...
    },
    {
      id: 'supernova-cyan',
      name: 'Supernova Cyan',
      desc: 'Electric neon cyan & vibrant teal',
    ...
    },
    {
      ...
      desc: 'Warm stellar amber & galactic gold',
    ...
    },
    {
      ...
      desc: 'Ultra-dark stealth obsidian & slate',
    ...
    },
    {
      id: 'emerald-aurora',
      name: 'Emerald Aurora',
      desc: 'Radiant northern lights emerald & mint',
  ...
  const resolutionPresets = [
    { label: '1080p (Full HD)', width: 1920, height: 1080, desc: 'Recommended Standard' },
  ...
  const handleFinish = async () => {
    ...
    if (authChoice === 'cracked' && username.trim()) {
      ...
      } else if (skinOption === 'cosmic') {
        finalSkin = `https://minotar.net/skin/${username.trim()}`;
      }
      await onCreateOfflineAccount(username.trim(), finalSkin, modelType);
    } else if (authChoice === 'microsoft') {
      onAddMicrosoftAccount();
    }
      ...
      <div className="relative w-full max-w-2xl bg-galaxy-900/90 border border-white/[0.12] rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
                ...
                {themes.map((t) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border transition-all cursor-pointer flex items-center justify-between bg-gradient-to-r ${t.gradient} ${
                        isSelected
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {resolutionPresets.map((res) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-xs font-semibold">{res.width} × {res.height}</div>
              ...
              {authChoice === 'microsoft' && (
                <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-200 space-y-1.5">
                  <div className="font-bold flex items-center space-x-1.5">
                    ...
                    <span>Microsoft Device Login</span>
                  </div>
                  <p className="text-[11px] text-slate-300 leading-relaxed">
                    When you finish, you can sign in with your Microsoft account via browser device code or add it anytime in Skins & Auth.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="px-8 py-4 bg-black/40 border-t border-white/[0.08] flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={handleNext}
              disabled={step === 4 && authChoice === 'cracked' && !username.trim()}
              className={`px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-cyan-500 hover:from-purple-500 hover:to-cyan-400 text-white font-bold text-xs shadow-glow-sm flex items-center space-x-2 transition-all ${
                step === 4 && authChoice === 'cracked' && !username.trim()
